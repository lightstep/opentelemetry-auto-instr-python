...
# 3p
import pymongo
from oteltrace.vendor.wrapt import ObjectProxy
...
def trace_mongo_client(client, tracer, service=mongox.TYPE):
    ...
    oteltrace.Pin(service=service, tracer=tracer).onto(traced_client)
...
class TracedMongoClient(ObjectProxy):
    ...
    def __init__(self, client=None, *args, **kwargs):
        ...
        # Default Pin
        oteltrace.Pin(service=mongox.TYPE, app=mongox.TYPE, app_type=AppTypes.db).onto(self)

    def __setddpin__(self, pin):
        pin.onto(self._topology)

    def __getddpin__(self):
        return oteltrace.Pin.get_from(self._topology)
...
class TracedTopology(ObjectProxy):
    ...
    def select_server(self, *args, **kwargs):
        ...
        oteltrace.Pin.get_from(self).onto(s)
        return s
...
class TracedServer(ObjectProxy):
    ...
    def _opentelemetry_trace_operation(self, operation):
        cmd = None
    ...
    def run_operation_with_response(self, sock_info, operation, *args, **kwargs):
        span = self._opentelemetry_trace_operation(operation)
        if not span:
