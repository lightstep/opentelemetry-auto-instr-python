...
class OpenTelemetrySampler(BaseSampler):
    """
    ...
    def __init__(self, rules=None, default_sample_rate=1.0, rate_limit=DEFAULT_RATE_LIMIT, _priority_sampler=None):
        """
        Constructor for OpenTelemetrySampler sampler
        ...
        for rule in rules:
            if not isinstance(rule, SamplingRule):
                raise TypeError('Rule {!r} must be a sub-class of type oteltrace.sampler.SamplingRules'.format(rule))
        self.rules = rules
...
class SamplingRule(object):
    """
    Definition of a sampling rule used by :class:`OpenTelemetrySampler` for applying a sample rate on a span
    """
