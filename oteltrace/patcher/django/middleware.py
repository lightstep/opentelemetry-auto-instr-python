...
EXCEPTION_MIDDLEWARE = 'oteltrace.contrib.django.TraceExceptionMiddleware'
TRACE_MIDDLEWARE = 'oteltrace.contrib.django.TraceMiddleware'
...
def _get_req_span(request):
    """ Return the opentelemetry span from the given request. """
    return getattr(request, '_opentelemetry_request_span', None)


def _set_req_span(request, span):
    """ Set the opentelemetry span on the given request. """
