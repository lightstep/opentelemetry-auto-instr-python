...
def _wrap_submit(func, instance, args, kwargs):
    ...
    current_ctx = None
    if oteltrace.tracer.context_provider._has_active_context():
        current_ctx = oteltrace.tracer.context_provider.active()
...
def _wrap_execution(ctx, fn, args, kwargs):
    ...
    """
    if ctx is not None:
        oteltrace.tracer.context_provider.activate(ctx)
