...
from oteltrace.patcher.base_patcher.base_patcher import BasePatcher
...
from .helpers import (
    get_current_app, get_current_span, simple_tracer, with_instance_pin
)
...
config._add('flask', dict(
    ...
    # DEV: Environment variable 'OPENTELEMETRY_SERVICE_NAME' used for backwards
    # compatibility
    ...
    # We mark 5xx responses as errors, these codes are additional status codes
    # to mark as errors
    # DEV: This is so that if a user wants to see `401` or `403` as an error,
    # they can configure that
...
class FlaskPatcher(BasePatcher):
    ...
    def patch(self):
        ...
        # flask.app.Flask methods that have custom tracing (add metadata,
        # wrap functions, etc)
        _w('flask', 'Flask.wsgi_app', traced_wsgi_app)
        _w(
            'flask',
            'Flask.dispatch_request', request_tracer('dispatch_request')
        )
        _w(
            'flask',
            'Flask.preprocess_request', request_tracer('preprocess_request')
        )
        _w('flask', 'Flask.add_url_rule', traced_add_url_rule)
        _w('flask', 'Flask.endpoint', traced_endpoint)
        _w(
            'flask',
            'Flask._register_error_handler', traced_register_error_handler
        )

        # flask.blueprints.Blueprint methods that have custom tracing (add
        # metadata, wrap functions, etc)
        ...
        for name in flask_app_traces:
            _w(
                'flask',
                'Flask.{}'.format(name), simple_tracer('flask.{}'.format(name))
            )
        ...
        if config.flask['trace_signals']:
            ...
            for signal in signals:
                ...
                # v0.9 missed importing `appcontext_tearing_down` in
                # `flask/__init__.py`
                ...
                if flask_version <= (0, 9) and (
                    signal == 'appcontext_tearing_down'
                ):
                    module = 'flask.signals'

                # DEV: Patch `receivers_for` instead of `connect` to ensure we
                # don't mess with `disconnect`
                _w(
                    module,
    ...
    def unpatch():
        ...
        for prop in props:
            ...
            # v0.9.0 missed importing `appcontext_tearing_down` in
            # `flask/__init__.py`
            #  https://github.com/pallets/flask/blob/0.9/flask/__init__.py#L35-L37
            #  https://github.com/pallets/flask/blob/0.9/flask/signals.py#L52
            # DEV: Version 0.9 doesn't have a patch version
            if flask_version <= (0, 9) and prop == (
                'appcontext_tearing_down.receivers_for'
            ):
                obj = flask.signals
...
def traced_wsgi_app(pin, wrapped, instance, args, kwargs):
    ...
    # Create a werkzeug request from the `environ` to make interacting with it
    # easier
    ...
    with pin.tracer.trace(
        ...
        # DEV: We tried using `Flask.finalize_request`, which seemed to work,
        # but gave us hell during tests
        # DEV: The downside to using `start_response` is we do not have a
        # `Flask.Response` object here,
        #   only `status_code`, and `headers` to work with
        #   On the bright side, this works in all versions of Flask (or any
        # WSGI app actually)
        def _wrap_start_response(func):
            def traced_start_response(status_code, headers):
                ...
                # Override root span resource name to be `<method> 404` for 404
                # requests
                # DEV: We do this because we want to make it easier to see all
                # unknown requests together
                #      Also, we do this to reduce the cardinality on unknown
                # urls
                # DEV: If we have an endpoint or url rule tag, then we don't
                # need to do this,
                #      we still want `GET /product/<int:product_id>` grouped
                # together,
                #      even if it is a 404
        ...
        # DEV: Use `request.base_url` and not `request.url` to keep from
        # leaking any query string parameters
        s.set_tag(http.URL, request.base_url)
        s.set_tag(http.METHOD, request.method)
        if config.flask.trace_query_string:
            s.set_tag(
                http.QUERY_STRING, compat.to_unicode(request.query_string)
            )
...
def traced_blueprint_register(wrapped, instance, args, kwargs):
    ...
    # Check if this Blueprint has a pin, otherwise clone the one from the app
    # onto it
...
def traced_add_url_rule(wrapped, instance, args, kwargs):
    """Wrapper for flask.app.Flask.add_url_rule to wrap all views attached to
    this app"""
    def _wrap(rule, endpoint=None, view_func=None, **kwargs):
        if view_func:
            # TODO: `if hasattr(view_func, 'view_class')` then this was
            # generated from a `flask.views.View`
            #   should we do something special with these views? Change the
            # name/resource? Add tags?
            view_func = wrap_function(
                instance, view_func, name=endpoint, resource=rule
            )
...
def traced_endpoint(wrapped, instance, args, kwargs):
    """Wrapper for flask.app.Flask.endpoint to ensure all endpoints are
    wrapped"""
    ...
    def _wrapper(func):
        ...
        return wrapped(endpoint)(
            wrap_function(instance, func, resource=endpoint)
        )
    return _wrapper


def traced_flask_hook(wrapped, instance, args, kwargs):
    """Wrapper for hook functions (before_request, after_request, etc) are
    properly traced"""
...
def traced_render_template_string(wrapped, instance, args, kwargs):
    ...
    with pin.tracer.trace(
        'flask.render_template_string', span_type=http.TEMPLATE
    ):
        return wrapped(*args, **kwargs)
...
def traced_render(wrapped, instance, args, kwargs):
    ...
    def _wrap(template, context, app):
        name = getattr(template, 'name', None) or (
            config.flask.get('template_default_name')
        )
        span.resource = name
...
def traced_register_error_handler(wrapped, instance, args, kwargs):
    """Wrapper to trace all functions registered with
    flask.app.register_error_handler"""
...
def request_tracer(name):
    ...
    def _traced_request(pin, wrapped, instance, args, kwargs):
        """
        Wrapper to trace a Flask function while trying to extract endpoint
        information
          (endpoint, url_rule, view_args, etc)

        This wrapper will add identifier tags to the current span from
        ...
        try:
            ...
            # DEV: This name will include the blueprint name as well (e.g.
            # `bp.index`)
            if not span.get_tag(FLASK_ENDPOINT) and request.endpoint:
                span.resource = u'{} {}'.format(
                    request.method, request.endpoint
                )

                span.set_tag(FLASK_ENDPOINT, request.endpoint)

            if not span.get_tag(FLASK_URL_RULE) and request.url_rule and (
                request.url_rule.rule
            ):
                span.resource = u'{} {}'.format(
                    request.method, request.url_rule.rule
                )
                span.set_tag(FLASK_URL_RULE, request.url_rule.rule)

            if not span.get_tag(FLASK_VIEW_ARGS) and request.view_args and (
                config.flask.get('collect_view_args')
            ):
                for k, v in request.view_args.items():
                    span.set_tag(u'{}.{}'.format(FLASK_VIEW_ARGS, k), v)
        except Exception as e:
            log.debug(
                'failed to set tags for "flask.request" span: {}'.format(e)
            )
...
def traced_signal_receivers_for(signal):
    """Wrapper for flask.signals.{signal}.receivers_for to ensure all signal
    receivers are traced"""
