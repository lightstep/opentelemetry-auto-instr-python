...
def patch():
    if getattr(sqlalchemy.engine, '__opentelemetry_patch', False):
        return
    setattr(sqlalchemy.engine, '__opentelemetry_patch', True)
...
def unpatch():
    # unpatch sqlalchemy
    if getattr(sqlalchemy.engine, '__opentelemetry_patch', False):
        setattr(sqlalchemy.engine, '__opentelemetry_patch', False)
